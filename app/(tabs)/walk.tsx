  ...
  useEffect(() => {
    ...
    const calculateTimeRemaining = () => {
      ...
      const rotationInterval =
        (walk.durationMinutes / walk.numberOfRotations) * 60 * 1000;
...
export default function WalkScreen() {
  ...
  const isUserCheckedIn =
    currentWalk?.checkedInUsers?.includes(user?.id || "") || false;
    ...
    return (
      <View style={styles.container}>
        ...
        <Text style={styles.subtitle}>
          Check in to get matched with a walking partner!
        </Text>
        <TouchableOpacity
  ...
  if (!userPair) {
    return (
      <View style={styles.container}>
        ...
        <Text style={styles.subtitle}>
          You'll be matched with a partner soon. Please wait for the next
          rotation.
        </Text>
    ...
    <View style={[styles.container, { backgroundColor }]}>
      ...
      <View style={styles.contentContainer}>
        <Text style={styles.title}>
          Find Your Walking Partner{userPair.users.length > 2 ? "s" : ""}!
        </Text>

        <View style={styles.pairInfo}>
          <Text style={styles.pairNumber}>{userPair.number}</Text>
          {userPair.users.length > 2 && (
            <Text style={styles.tripleIndicator}>
              Group of {userPair.users.length}
            </Text>
          )}
        </View>

        <Text style={styles.instructions}>
          Look for {userPair.users.length > 2 ? "others" : "someone"} with the
          same number and color, and start walking together!
        </Text>
